// 개발용: 스티커 16종을 만화 얼굴에 얹은 미리보기 PNG를 만든다(위치·크기 눈 확인용). 환경변수 EFFECT_PREVIEW_DIR이 있을 때만 실행.
import CoreImage
import UIKit
import XCTest
...
final class EffectPreviewTests: XCTestCase {
    func testRenderStickerPreviewSheet() throws {
        ...
        for (i, kind) in kinds.enumerated() {
            ...
            for layer in EffectRenderer.stickerLayers(kind, face: anchors, time: 0.6) { cellImage = layer.composited(over: cellImage) }
