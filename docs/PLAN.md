...
| **A4 라이브 보정 골격** | `AVCaptureVideoDataOutput` → `CIImage` → `MTKView` 렌더 루프, 프리셋 실시간 적용, 인물 모드 스위치·프리셋 스트립 UI, 촬영 후 풀해상도 후처리 → A2 경로로 저장 | `Capture/MetalPreviewView.swift`, `Capture/LivePipeline.swift` |
| **A5 인물 보정 골격** | 인물 모드 켜짐 시 Vision 랜드마크 → 피부 마스크 생성, 하이패스 피부 보정 필터, 메시 워프 Metal 셰이더(턱선·광대·눈), 파이프라인 3단계에 연결. 라이브는 경량(양방향 필터·저해상도 메시), 저장은 풀 품질 | `Portrait/` 소스, Metal 셰이더 |
