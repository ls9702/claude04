...
- **한 단계 = 작성 + 리뷰 한 세트.** 이 클라우드 세션이 코드를 작성하고(Opus 5.5), 이어서 같은 세션에서 체크리스트 리뷰(Fable)까지 마친 뒤 커밋한다. Mac 세션은 빌드·설치·실기기 관찰만 한다. 절차는 §11.
...
| 09-25 | **v1.0 확정.** 릴리즈 1 = 카메라 모드, 릴리즈 2 = 동영상 모드. 단계별 진행·주 단위 재개. **각 단계는 작성(Opus 5.5)+리뷰(Fable)를 이 세션에서 한 세트로**, Mac은 빌드만 |
...
## 11. 협업 방식 (클라우드 세션 = 작성+리뷰 · Mac 세션 = 빌드)

### 11.1 역할
| 세션 | 하는 일 |
|---|---|
| **클라우드 세션 (이 세션)** | 단계마다 **작성 → 리뷰**를 한 세트로 수행. 작성은 Opus 5.5, 리뷰는 Fable로 모델을 바꿔 진행(사용자가 `/model`로 전환). 리뷰는 `docs/REVIEW_CHECKLIST.md` 기준, 결과는 `docs/REVIEW_LOG.md`. 리뷰에서 나온 수정까지 반영해 커밋·푸시하고 HANDOFF 갱신 |
| **Mac 세션** | 빌드 체크포인트에서 `scripts/build.sh`·`test.sh`·`install-device.sh`, 컴파일 오류 최소 수정, `docs/BUILD_LOG.md` 기록, 실기기 관찰. 기능 코드·설계는 손대지 않음 |

### 11.2 단계 진행 절차 (토큰 소진·주 단위 재개 대응)
```
① 클라우드: HANDOFF "다음 시작 지점" 읽기 → 작성 (코드+테스트, 30분마다 wip 커밋)
② 클라우드: 같은 단계를 REVIEW_CHECKLIST로 리뷰 → REVIEW_LOG 기록 → 지적 사항 수정
③ 클라우드: 커밋 `feat(R1-S1): …` 푸시 → HANDOFF "단계 상태" 작성·리뷰 완료, "다음 시작 지점" = 다음 단계
④ 빌드 체크포인트면 Mac: 빌드·설치 → BUILD_LOG. 실패 → 클라우드가 다음 턴에 수정
⑤ 다음 단계 ①
```
- 토큰 소진 시 마지막 커밋이 재개 지점. 다음 세션은 `git log -5`와 HANDOFF만 보고 이어간다. 작성 중 끊기면 "작성 중", 리뷰 중 끊기면 "리뷰 중"으로 HANDOFF에 남긴다.
