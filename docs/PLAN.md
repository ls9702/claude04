...
- **라이브**: 프리뷰에 선택한 프리셋(+ 인물 모드가 켜져 있으면 인물 보정)을 **실시간 적용해 보면서 촬영**한다(저해상도, Metal, 30fps). 셔터를 누르면 Apple ISP 처리 원본을 받아 **같은 설정을 풀해상도로 후처리** → 원본과 함께 사진 앱에 저장(비파괴). 프리뷰와 저장본은 같은 파이프라인이므로 보이는 대로 저장되며, 저장본이 해상도만 더 높다.
