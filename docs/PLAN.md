...
작성일: 2026-09-25 · 상태: **v1.0 확정 — 릴리즈 1(카메라) 단계 진행** · 결정 이력은 §10
...
### 8.1 원칙
- **릴리즈 1 = 카메라(사진) 모드**, **릴리즈 2 = 동영상(쇼츠) 모드**. 릴리즈 1이 iPhone에서 실사용 가능해진 뒤에 릴리즈 2를 시작한다.
- **한 단계 = 한 세션 분량.** 각 단계는 시작 조건·산출물·완료 기준·리뷰 포인트가 정해져 있고, 끝나면 반드시 커밋·푸시 + `docs/HANDOFF.md` 갱신. 토큰이 소진되면 다음 주에 새 세션이 HANDOFF의 "다음 시작 지점"에서 이어간다.
- 세션 역할: **개발 = Opus 5.5**, **리뷰 = Fable**, **빌드·실기기 = Mac 세션**. 절차는 §11.
- 이 환경에서는 컴파일이 안 되므로 각 단계 산출물은 "작성 완료"이고, 표시된 **빌드 체크포인트**에서 Mac 세션이 검증한다.
...
| 09-25 | **v1.0 확정.** 릴리즈 1 = 카메라 모드, 릴리즈 2 = 동영상 모드. 단계별 진행·주 단위 재개. 개발 Opus 5.5 / 리뷰 Fable / 빌드 Mac |
| 09-25 | 확인: 앱 시작 = 카메라, **인물 모드는 수동 토글**(자동 아님), 라이브 보정을 보면서 촬영. §3.1·§3.3·§5 반영 |
| 09-25 | **v0.5.** 사진 보정이 주 기능, 쇼츠는 보조. 결과는 사진 보관함에 저장하고 EXIF·GPS·날짜 보존(§3.4). 실행 계획을 Mac 빌드 전(A)·후(B)로 분리, 사진 먼저 |

## 11. 세션 협업 방식 (개발 Opus 5.5 · 리뷰 Fable · Mac 빌드)

### 11.1 역할
| 세션 | 모델/환경 | 하는 일 | 하지 않는 일 |
|---|---|---|---|
| **개발 세션** | Opus 5.5 (클라우드) | 단계 하나를 맡아 코드 작성, 단위 테스트, 커밋 `feat(R1-S1): …`, HANDOFF에 "리뷰 대기" 기록 | 설계 변경(필요하면 HANDOFF "논의 필요"에 적고 계속), 여러 단계 동시 진행 |
| **리뷰 세션** | Fable (클라우드) | 리뷰 대기 단계의 diff를 §11.3 체크리스트로 검토 → `docs/REVIEW_LOG.md`에 결과 기록. 작은 문제는 직접 수정 커밋 `fix(R1-S1): …`, 큰 문제는 개발 세션에 되돌림. 계획서 관리 | 새 기능 작성 |
| **Mac 세션** | Mac Claude Code | 빌드 체크포인트에서 `scripts/build.sh`·`test.sh`·`install-device.sh`, 컴파일 오류 최소 수정, `docs/BUILD_LOG.md` 기록, 실기기 관찰·튜닝 값 제안 | 설계 변경 |

### 11.2 단계 진행 절차 (토큰 소진·주 단위 재개 대응)
```
① 개발 세션: HANDOFF "다음 시작 지점" 읽기 → 해당 단계 착수 → 코드+테스트 → 커밋·푸시
             → HANDOFF: 단계 상태 "리뷰 대기", "다음 시작 지점" = 다음 단계
② 리뷰 세션: HANDOFF "리뷰 대기" → diff 리뷰 → REVIEW_LOG 기록 → 통과면 "리뷰 완료", 아니면 "수정 필요"+항목
③ (수정 필요) 개발 세션이 반영 → 다시 ②
④ 빌드 체크포인트 단계면 Mac 세션이 빌드·설치 → BUILD_LOG → 실패면 개발 세션이 수정
⑤ HANDOFF "단계 상태" ✅ → 다음 단계 ①
```
- 세션이 도중에 끊기면(토큰 소진) **마지막 커밋이 재개 지점**이다. 작업 중 파일은 30분마다 `wip(R1-S1): …`로 커밋해 잃지 않는다. 다음 세션은 `git log`와 HANDOFF만 보고 이어간다.
- 새 세션 첫 지시 예: "CLAUDE.md와 docs/HANDOFF.md를 읽고 '다음 시작 지점' 단계를 진행해." / 리뷰 세션: "HANDOFF의 리뷰 대기 단계를 REVIEW_CHECKLIST로 리뷰해."

### 11.3 리뷰 체크리스트 (`docs/REVIEW_CHECKLIST.md`)
필수(하나라도 어기면 수정 필요): 사진 보관함 저장·메타데이터 보존 원칙 / 외부 패키지 추가 없음 / 세션 큐·메인 큐 분리 / 원본 손상 경로 없음 / 권한 거부 시 크래시 없음.
품질: 단계 완료 기준 충족 / 테스트 존재·의미 / 큰 이미지 메모리 / 오류 처리 / 한국어 문자열 / 설계(§3·§4)와 일치.

### 11.4 연결 파일
`CLAUDE.md`(규약), `docs/HANDOFF.md`(상태·다음 시작 지점), `docs/REVIEW_LOG.md`(리뷰 결과), `docs/BUILD_LOG.md`(빌드 결과), `scripts/*.sh`(Mac 명령), `.claude/settings.json`(권한).

## 12. 다음 행동
1. **개발 세션(Opus 5.5)** 시작: "CLAUDE.md와 docs/HANDOFF.md를 읽고 '다음 시작 지점'(R1-S1 보정 엔진)을 진행해."
2. **Mac 세션** 시작: R1-S0 첫 빌드·설치 → BUILD_LOG.
3. R1-S1 커밋 후 **리뷰 세션(Fable)**: "HANDOFF의 리뷰 대기 단계를 리뷰해."
4. 이후 §11.2 절차 반복. 릴리즈 1 완료 후 릴리즈 2.
