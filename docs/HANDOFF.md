...
## ▶ 다음 시작 지점
**클라우드 세션 — R1-S8 릴리즈 1 마무리 착수 가능**: 실기기 첫 설치·관찰 완료(`docs/BUILD_LOG.md` 09-26 14:30). 반영할 것: ① **라이브 프리뷰 성능** — iPhone 12 Pro에서 `20fps · 버림 11+1 · 768px`(열 serious), 발열 심함 → BUILD_LOG "튜닝 제안" 검토. ② **사용자 추가 요청 2건**(아래 "논의 필요") — R1-S8 범위에 넣을지 결정. ③ 야경 저장(S7)은 야간 미측정이라 R1-S8 중 Mac이 재측정.
