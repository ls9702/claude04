...
## ▶ 다음 시작 지점
**클라우드 세션**: **R1-S8b 인물 모드 기능·UX** — Opus 5.5 서브에이전트 작성 → Fable 리뷰 → 커밋. 범위는 아래 "논의 필요"의 [결정 09-26] 8건. 그다음 **R1-S8c**(백업·열/배터리 경고·오류 처리·앱 아이콘·전체 리뷰 → **릴리즈 1**).
**Mac 세션**: 빌드 체크포인트 **R1-S8a** — `scripts/build.sh` → `scripts/test.sh` → `scripts/install-device.sh` → 아래 "Mac 세션이 할 일"(S8a 재측정). 야간에 야경 프리셋 저장(S7)도 함께.

## 진행 중
| 세션 | 단계 | 상태 | 시작 | 메모 |
|---|---|---|---|---|
| 클라우드 | R1-S8b | 작성 중 (Opus 5.5 서브에이전트 위임) | 09-26 | 인물 모드: 강도 칩·배경 흐림·라이브 전/후·마커·패널 정리·인물 버튼 이동·얼굴 확대·전면 전환 제안 |
...
| R1-S8b 인물 모드 기능·UX | 작성 중 | | |
| R1-S8c 마무리 | 대기 | | **릴리즈 1** |
| R2-S1 ~ R2-S5 | 릴리즈 1 이후 | | **릴리즈 2** |

## Mac 세션이 할 일 (빌드 체크포인트 R1-S8a)
1. `git pull` → `scripts/build.sh` → 오류는 `docs/REVIEW_LOG.md` S8a "컴파일 확신이 낮은 지점"부터 → `fix(R1-S8a): …`
2. `scripts/test.sh` (약 140개 예상)
3. `scripts/install-device.sh` → iPhone 12 Pro:
   - 콘솔 로그: "활성 포맷 1920x1440 … 사진 4032x3024" / "프리뷰 프레임 크기 1440x1920" / "촬영 사진 크기 4032x3024" / "카메라 … 전환 계수 [2.0, 4.0]". 폴백 로그가 나오면 기록
   - DEBUG 통계: nominal에서 fps(기대 28~30, 버림 0~2), 10분 후 열 상태·발열(이전 20fps@768·serious와 비교)
   - 렌즈 0.5×/1×/2× 버튼·시작 1×·핀치 0.5~10·자동 전환 튐, 전면 전환(프리뷰 미러, 저장 사진 비반전, 탭 노출 위치, 가로 셀피 방향)
   - 피부톤 슬라이더 50·100(따뜻한 방향), 라이브 인물 모드 fps(1명/여럿), 마스크 경계, 윤곽 20·100 배경 휨
   - 썸네일 탭 → 사진 앱, 보정 탭 닫기·확인 알림, 10장 일괄 저장 시간
   - 야간: 야경 프리셋 저장 3초 이내·품질·모델 출력 0 아님, 야간 프리뷰 밝기(프레임 간격 수정 후)
4. 튜닝 값(`PreviewQuality`, `autoRefreshInterval`, `WarpPlan`·`SkinSmoothing` 상수, `preferredFrameRate`)은 직접 바꿔 커밋 가능. BUILD_LOG·이 파일 갱신
